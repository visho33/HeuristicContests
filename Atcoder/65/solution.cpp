...
int main(){
    ...
    for(int i = 0; i<N; i++){
        for(int j = i+1; j<N; j++){
            ...
            ll xj = (rx[j] + lx[j]);
            ll yj = (ry[j] + ly[j]);
            distancia[i][j] = (xi - xj)*(xi - xj) + (yi - yj)*(yi - yj);
    ...
    while(true){
        ...
        for(int i = 0; i<N; i++){
            ...
            if(mapa.count(subtree[i]) == 0){
                mapa[subtree[i]] = i;
            }
            int x = mapa[subtree[i]];
            int c1 = 0, c2 = 0;
            if(i != inicio){
                c1 = dis(i, padre[i]);
            }
        ...
        for(int j = 0; j<M; j++){
            ...
            //    c = dis(x, padre[x]);
            //}
            if(mapa.count(G[j]) != 0 && G[j] < mini){
                mini = G[j];
                where = j;
                alreves = false;
                break;
            }
            if(mapa.count(tot - G[j]) != 0 && G[j] < mini){
                mini = G[j];
                where = j;
                alreves = true;
                break;
        ...
        if(alreves == false){
            ...
            colorear(padre[mapa[tot - G[where]]], mapa[tot - G[where]], where); 
        }
    }
    ...
    cerr<<fixed<<setprecision(0)<<score2<<" "<<lol<<endl;
